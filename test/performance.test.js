...
describe('Performance Tests', () => {
    describe('Filtering Algorithm Performance', () => {
        test('should filter large arrays of titles efficiently', () => {
            ...
            for (let i = 0; i < 10000; i++) {
                if (i % 5 === 0) {
                    ...
                    testTitles.push(
                        `You won't believe what happened in video ${i}`
                    );
                } else {
            ...
            console.log(
                `Processed ${testTitles.length} titles in ${processingTime.toFixed(2)}ms`
            );
            console.log(`Filtered ${filteredTitles.length} clickbait titles`);
            console.log(
                `Processing rate: ${((testTitles.length / processingTime) * 1000).toFixed(0)} titles/second`
            );
        ...
        test('should perform word matching efficiently with various string lengths', () => {
            ...
            const mediumTitle =
                'This SHOCKING discovery will absolutely BLOW your mind and change everything';
            const longTitle =
                'This is an extremely long title that contains many words and phrases that we need to test for performance when checking against our SHOCKING clickbait detection system which should work efficiently even with very long strings that contain multiple potential matches';
            ...
            titles.forEach((title) => {
                ...
                console.log(
                    `Average processing time for ${title.length} char title: ${avgTime.toFixed(4)}ms`
                );
    ...
    describe('DOM Manipulation Performance', () => {
        test('should handle frequent DOM queries efficiently', () => {
            ...
            const mockElements = Array.from({ length: 100 }, (_, i) => ({
                ...
                querySelector: () => ({
                    textContent:
                        i % 3 === 0 ? 'SHOCKING content' : 'Normal content',
                    style: {},
                }),
                style: { display: '' },
            }));
            ...
            mockElements.forEach((element) => {
                ...
                const shouldHide = CLICKBAIT_WORDS.some((word) =>
            ...
            console.log(
                `Processed ${mockElements.length} DOM elements in ${processingTime.toFixed(2)}ms`
            );
    ...
    describe('Regex Performance', () => {
        test('should compare regex vs string methods for uppercase detection', () => {
            ...
            console.log(
                `Performance ratio: ${(regexTime / stringTime).toFixed(2)}x`
            );
    ...
    describe('Array Search Optimization', () => {
        test('should measure performance of different search strategies', () => {
            const testTitle =
                'this shocking video will blow your mind and change everything forever';
            ...
            for (let i = 0; i < iterations; i++) {
                CLICKBAIT_WORDS.find((word) => testTitle.includes(word)) !==
                    undefined;
            }
    ...
    describe('Real-world Simulation', () => {
        test('should handle typical YouTube page load simulation', () => {
            ...
            const filteredCount = filteredVideos.filter(
                (v) => v.filtered
            ).length;
            const regularCount = filteredVideos.length - filteredCount;

            console.log(
                `Processed ${youtubeSimulation.length} video titles in ${processingTime.toFixed(2)}ms`
            );
            console.log(
                `Filtered ${filteredCount} clickbait videos, kept ${regularCount} regular videos`
            );
